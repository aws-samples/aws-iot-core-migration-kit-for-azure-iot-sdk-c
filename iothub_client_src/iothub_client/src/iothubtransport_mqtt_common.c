...
#ifdef IOT_CORE_PATCH
static const char TOPIC_DEVICE_METHOD_PREFIX[] = "device/";
#else
...
#ifdef IOT_CORE_PATCH
static const char* REQUEST_ID_PROPERTY = "";
#else
...
#ifdef IOT_CORE_PATCH
static STRING_HANDLE iot_core_wrapper_mqttmessage_getTopicName(PMQTTTRANSPORT_HANDLE_DATA transportData, MQTT_MESSAGE_HANDLE msgHandle, bool *known_topic);
static APP_PAYLOAD* iot_core_wrapper_mqttmessage_getApplicationMsg(MQTT_MESSAGE_HANDLE, DEVICE_TWIN_UPDATE_STATE update_state);
#endif
...
#ifdef IOT_CORE_PATCH
    STRING_HANDLE msg_topic = STRING_construct_sprintf("$aws/things/%s/shadow/get", STRING_c_str(transport_data->device_id));
#else
...
#ifdef IOT_CORE_PATCH
    STRING_HANDLE msgTopic = STRING_construct_sprintf("$aws/things/%s/shadow/update", STRING_c_str(transport_data->device_id));
#else
    ...
    if (msgHandle != NULL && callbackCtx != NULL)
    {
        ...
        else
        {
            ...
            if (type == IOTHUB_TYPE_DEVICE_TWIN)
            {
                ...
                {
#ifdef IOT_CORE_PATCH
                    APP_PAYLOAD* payload = iot_core_wrapper_mqttmessage_getApplicationMsg(msgHandle, (notification_msg) ? DEVICE_TWIN_UPDATE_PARTIAL : DEVICE_TWIN_UPDATE_COMPLETE);
#else
...
#ifdef IOT_CORE_PATCH
            STRING_delete(topic_resp_modified);
#endif
        }
    }
}
...
int IoTHubTransport_MQTT_Common_Subscribe_DeviceTwin(TRANSPORT_LL_HANDLE handle)
{
    ...
    {
#ifdef IOT_CORE_PATCH
        subscribeToNotifyStateIfNeeded(transport_data);
#endif
...
#ifdef IOT_CORE_PATCH
            transport_data->topic_GetState = STRING_construct_sprintf("$aws/things/%s/shadow/get/#", STRING_c_str(transport_data->device_id));
#else
...
static int InternStrEndWith(const char * str, const char * suffix)
{
...
static uint16_t iot_core_get_rid(MQTT_MESSAGE_HANDLE msgHandle)
{
    uint16_t rid = 0;
    JSON_Value* root_value = NULL;
    JSON_Object* root_object = NULL;
    JSON_Value* json_clientToken;
    ...
    json_clientToken = json_object_dotget_value(root_object, "clientToken");
    if (json_clientToken != NULL)
    {
        clientToken = json_serialize_to_string(json_clientToken);
        if (clientToken != NULL)
        {
            rid = strtoul(clientToken+1, NULL, 10);
            free(clientToken);
        }
    }
...
static APP_PAYLOAD* iot_core_wrapper_mqttmessage_getApplicationMsg(MQTT_MESSAGE_HANDLE msgHandle, DEVICE_TWIN_UPDATE_STATE update_state)
{
    JSON_Value* root_value = NULL;
    JSON_Object* root_object = NULL;
    JSON_Value* json_reported;
    JSON_Value* json_desired;
    JSON_Value* json_version;
    char* desired = NULL;
    char* reported = NULL;
    int version = 1;
    ...
    json_desired = json_object_dotget_value(root_object, "state.desired");
    if (json_desired == NULL)
    {
        json_desired = json_object_dotget_value(root_object, "current.state.desired");
    }
    if (json_desired != NULL)
    {
        desired = json_serialize_to_string(json_desired);
    }

    json_reported = json_object_dotget_value(root_object, "state.reported");
    if (json_reported == NULL)
    {
        json_reported = json_object_dotget_value(root_object, "current.state.reported");
    }
    if (json_reported != NULL)
    {
        reported = json_serialize_to_string(json_reported);
    }

    json_version = json_object_dotget_value(root_object, "version");
    if (json_version == NULL)
    {
        json_version = json_object_dotget_value(root_object, "current.version");
    }
    if (json_version != NULL)
    {
        version = (int)json_value_get_number(json_version);
    }
    ...
    STRING_HANDLE iot_hub_json_handle;
    if (update_state == DEVICE_TWIN_UPDATE_COMPLETE)
    {
        iot_hub_json_handle = STRING_construct_sprintf("{\"desired\":{%.*s%s\"$version\":%d},\"reported\":{%.*s%s\"$version\":%d}}",
                                                                     (desired == NULL) ? 0 : strlen(desired) -2,
                                                                     (desired == NULL) ? "" : desired + 1,
                                                                     (desired == NULL) ? "" : ",",
                                                                     version,
                                                                     (reported == NULL) ? 0 : strlen(reported) -2,
    ...
    {
        iot_hub_json_handle = STRING_construct_sprintf("{%.*s%s\"$version\":%d}",
                                                                     (desired == NULL) ? 0 : strlen(desired) -2,
                                                                     (desired == NULL) ? "" : desired + 1,
                                                                     (desired == NULL) ? "" : ",",
                                                                     version
